            ...
            with open(file, "rt") as fin, self.subTest(file=file):
                ...
                for line in fin:
                    ...
                    if matchMessage is not None:
                        ...
                        numMessage += 1
                        self.assertFalse(
                            foundruleCount > 0 or lineruleCount > 0,
                            file
                            + f" in line {str(line_number-1)}: message should not have rules for '{statement.strip()}'",
                        )
                    ...
                    if isEnum:
                        ...
                        if matchName is not None:
                            ...
                            self.assertFalse(
                                foundruleCount > 0 or lineruleCount > 0,
                                file
                                + f" in line {str(line_number-1)}: enum field should not have rules for '{statement.strip()}'",
                            )
                    ...
                    if matchEnum is not None:
                        ...
                        if matchName is not None:
                            self.assertFalse(
                                foundruleCount > 0 or lineruleCount > 0,
                                file
                                + f" in line {str(line_number-1)}: enum should not have rules for '{statement.strip()}'",
                            )
                    ...
                    if matchComment is not None:
                        ...
                        if numMessage > 0:
                            if statement.find(";") != -1:
                                ...
                                self.assertFalse(
                                    hasRule and not endRule,
                                    file
                                    + " in line "
                                    + str(line_number)
                                    + ": \\endrules statement does not exists for: '"
                                    + statement
                                    + "'. Check spacing and rule syntax.",
                                )
                                self.assertFalse(
                                    not hasRule and endRule,
                                    file
                                    + " in line "
                                    + str(line_number)
                                    + ": \\rules statement does not exists for: '"
                                    + statement
                                    + "'. Check spacing and rule syntax.",
                                )
                                self.assertFalse(
                                    ...
                                    + " in line "
                                    + str(line_number)
                                    + ": rules found but no statements (\\rules and \\endrules) around it for: '"
                                    + statement
                                    + "'",
                                )
                                lineruleCount = 0
